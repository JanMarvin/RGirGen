...
resolve_out_param <- function(p, local_nm) {
  ...
  if (cls == "inttype") {
    entry <- p$type$int_entry
    decl  <- if (nonempty(entry$c_type)) entry$c_type else "gint"
    return(sprintf("  %s %s = 0; (void)%s;", decl, local_nm, local_nm))
  }
  ...
  if (cls == "scalar") {
    if (gi %in% STRING_GI_TYPES) {
      ...
      return(sprintf("  %s %s = %s; (void)%s;", base, local_nm, init, local_nm))
    }
    # Plain numeric scalar
    tm <- gi_type_to_map(gi)
    return(sprintf("  %s %s = 0; (void)%s;", tm$decl_type, local_nm, local_nm))
  }
  ...
  if (!nonempty(bare) || bare == "void")
    return(sprintf("  gpointer %s = NULL; (void)%s;", local_nm, local_nm))
  ...
  if (c_type_is_pointer(base)) {
    ...
    init <- if (bare %in% struct_types) "{0}" else "0"
  } else {
    init <- "0"
  }

  sprintf("  %s %s = %s; (void)%s;", base, local_nm, init, local_nm)
}
...
resolve_box <- function(ret_type, var_nm) {
  ...
  # If it's an OUT param and a scalar, box it immediately.
  # Do NOT use the NULL check logic intended for pointers.
  if (cls == "scalar" && grepl("^_out_", var_nm)) {
    map <- gi_type_to_map(gi)
    return(gsub("{{V}}", var_nm, map$box, fixed = TRUE))
  }

  # 1. Handle String Arrays (GStrv)
  if (gi == "GStrv" || grepl("\\*\\*", ct)) {
    return(sprintf("_box_GStrv((char**)%s)", var_nm))
  }

  bare <- trimws(sub("^const\\s+", "", strip_one_pointer(ct)))

  # 2. Structs that live on the stack and must be copied to the heap
  # Added GtkTextIter to this list
  ...
  if (bare %in% struct_types) {
    # If it's an OUT param (like _out_iter), it's a local variable.
    # Take its address and copy to heap.
    if (grepl("^_out_", var_nm)) {
      return(sprintf("make_boxed_struct(&%s, sizeof(%s))", var_nm, bare))
    }
    # If the return type was already a pointer to a struct, copy the data
    if (c_type_is_pointer(ct)) {
      return(sprintf("((%s) == NULL) ? R_NilValue : make_boxed_struct(%s, sizeof(%s))",
                     var_nm, var_nm, bare))
    }
    # Default: take address of local and copy
    return(sprintf("make_boxed_struct(&%s, sizeof(%s))", var_nm, bare))
  }
  ...
  # 3. Scalar handling (Numeric/Logical)
  if (cls == "scalar") {
    map <- gi_type_to_map(gi)
    box_expr <- map$box

    # Check if this mapping uses Rf_ScalarReal.
    # If so, we need the size_t bridge to protect against pointer-to-double casts.
    if (grepl("Rf_ScalarReal", box_expr)) {
      # Transform '((double)({{V}}))' into '((double)(size_t)({{V}}))'
      box_expr <- gsub("(double)", "(double)(size_t)", box_expr, fixed = TRUE)
    }

    return(gsub("{{V}}", var_nm, box_expr, fixed = TRUE))
  }

  # 4. GObject handling (The logic for GtkTextTag, GtkTextBuffer, etc.)
  ...
  # 5. Default Pointer handling
  if (c_type_is_pointer(ct)) {
    return(sprintf("((%s) == NULL) ? R_NilValue : R_MakeExternalPtr((void*)(%s), R_NilValue, R_NilValue)",
                   var_nm, var_nm))
  } else {
    # If it's not a pointer (e.g. a raw struct on stack), wrap by address
...
generate_c_function <- function(fn) {
  ...
  for (p in fn$params) {
    ...
    if (isTRUE(p$nullable) && classify_type(p$type) == "pointer") {
      decls <- c(decls, sprintf(
        "  %s %s = (%s != R_NilValue) ? (%s)R_ExternalPtrAddr(%s) : NULL; (void)%s;",
        r$decl_type, v_nm, s_nm, r$decl_type, s_nm, v_nm))
    } else {
      decls <- c(decls, sprintf("  %s %s = (%s)(%s); (void)%s;",
                                r$decl_type, v_nm, r$decl_type, r$unbox_expr, v_nm))
    }
    call_args <- c(call_args, v_nm)
  }

  if (has_error) {
    decls     <- c(decls, "  GError *_err = NULL; (void)_err;")
  ...
  if (ret_gi == "none") {
    ...
    if (nchar(err_cl) > 0) body <- paste0(body, err_cl, "\n")

    if (length(out_params) == 1) {
      # Return the single out-param
      p_out <- out_params[[1]]
      box_expr <- resolve_box(p_out$type, paste0("_out_", p_out$name))
      body <- paste0(body, sprintf("  return %s;", box_expr))
    } else {
      body <- paste0(body, "  return R_NilValue;")
    }
  } else {
    # NON-VOID RETURN
    r_ret <- resolve_in_param(list(type=fn$return_type), "")

    # Use gconstpointer if the return type is a pointer to accommodate 'const' returns
    if (c_type_is_pointer(fn$return_type$c)) {
      ret_decl <- "gconstpointer"
    } else {
      ret_decl <- if (nonempty(r_ret$decl_type)) r_ret$decl_type else "gpointer"
    }

    body <- paste0(body, sprintf("  %s _ret = %s;\n", ret_decl, call_str))

    if (nchar(err_cl) > 0) body <- paste0(body, err_cl, "\n")

    # NULL check
    if (c_type_is_pointer(fn$return_type$c)) {
      body <- paste0(body, "  if (_ret == NULL) return R_NilValue;\n")
    }

    # Box the result
    box_expr <- resolve_box(fn$return_type, "_ret")
    body <- paste0(body, sprintf("  return %s;", box_expr))
  }
